function ParametricOrderedPreferencesMPCCGameHybrid(;
    objectives,
    ...
    function set_up_level(priority_level, player_idx)
        ...
        # Slacks are required for hybrid
        ...
        if !is_prioritized_constraint[player_idx][priority_level]
            push!(objectives_w_penalty, 5*objective_ii) #using penalty α = 50
    ...
    # Hybrid 
    fs = [map(f->f(trajectory_x,θ), objectives)[i] + objectives_w_penalty[i] for i in 1:num_players]
